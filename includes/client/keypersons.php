...
<!--                    <td>HQ Location</td>-->
            ...
            <tbody>
                ...
                <?php
                    ...
                    $results_per_page = 20;
                    $pick = mysqli_query($db, "SELECT * FROM keypersons_table");
                    $total_results = mysqli_num_rows($pick);
                    $total_page_num = ceil($total_results/$results_per_page);
                    $offset = ($num - 1) * $results_per_page;

                    $select = mysqli_query($db, "SELECT * FROM keypersons_table ORDER BY key_person ASC LIMIT $offset, $results_per_page");
