...
$select = mysqli_query($db, "SELECT * FROM keypersons_table WHERE (created_by = '{$_SESSION['email']}') && (clients_name LIKE '%$search%' || clients_email LIKE '%$search%' || clients_hq_location LIKE '%$search%' || clients_category LIKE '%$search%' || key_person LIKE '%$search%' || title LIKE '%$search%' || info LIKE '%$search%')");
