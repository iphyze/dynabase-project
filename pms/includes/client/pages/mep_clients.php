...
<div class="table-container">
    ...
    <div class="form-title">Mep Consultant Clients Data</div>
...
//
//                    $results_per_page = 20;
//                    $pick = mysqli_query($db, "SELECT * FROM clients_table WHERE clients_category = 'Mep Consultant'");
//                    $total_results = mysqli_num_rows($pick);
//                    $total_page_num = ceil($total_results/$results_per_page);
//                    $offset = ($num - 1) * $results_per_page;

                    $select = mysqli_query($db, "SELECT * FROM clients_table WHERE clients_category = 'Mep Consultant' && created_by = '{$_SESSION['email']}' ORDER BY clients_name ASC");
