...
<div class="table-container">
    ...
    <div class="form-title">General Contractor Clients Data</div>
...
//
//                    $results_per_page = 20;
//                    $pick = mysqli_query($db, "SELECT * FROM clients_table WHERE clients_category = 'Project_manager'");
//                    $total_results = mysqli_num_rows($pick);
//                    $total_page_num = ceil($total_results/$results_per_page);
                    //$offset = ($num - 1) * $results_per_page;

                    $select = mysqli_query($db, "SELECT * FROM clients_table WHERE clients_category = 'General Contractor' && created_by = '{$_SESSION['email']}' ORDER BY clients_name ASC");
