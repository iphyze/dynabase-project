<div class="home-heading">
    ...
    <div class="home-heading-col-1">
        ...
        Click or select the company's category whose record you want to view.    
        </div>
...
<div class="table-container">
    ...
    <div class="form-title">All Companies</div>
    <a class="form-title" href="./?page=add_client">+ Add Company</a><br>
    <a class="form-title" href="./?page=clients">&larr; Back</a>
...
-->
            ...
            <tbody>
                ...
                <?php
                    ...
                    $results_per_page = 20;
                    $pick = mysqli_query($db, "SELECT * FROM clients_table WHERE created_by = '{$_SESSION['email']}'");
                    $total_results = mysqli_num_rows($pick);
                    $total_page_num = ceil($total_results/$results_per_page);
                    $offset = ($num - 1) * $results_per_page;

                    $select = mysqli_query($db, "SELECT * FROM clients_table WHERE created_by = '{$_SESSION['email']}' ORDER BY created_at DESC LIMIT $offset, $results_per_page");
                    ...
                    if($number > 0){
                        foreach($select as $selected){
							...
							if($clients_category == "Mep Consultants"){
								$clients_category = "MEP Consultants";
							}
...
<!--                        <a href="delete.php?action=delete_client&id=<?php //echo $id; ?>" class="fas fa-trash action-delete" onclick="return confirm('Are you sure you want to delete this client data?')"></a>-->
                ...
                ?>
                <tr>
                    <td colspan="7">You haven't created any clients!</td>
                </tr>
                <?php
                }
                ?>
